...
export function updateStateAfterPlan(
  ...
  if (
    ...
    content.includes(' of ') &&
    content.includes(' in current phase')
  ) {
    content = content.replace(
      /Plan:\s*\d+\s+of\s+\d+\s+in current phase/,
      `Plan: ${plansCompleted} of ${totalPlansInPhase} in current phase`
    );
  }
 ...
 */
export function updateStateAfterPhaseComplete(
  statePath: string,
  phaseNum: number,
  totalPlansInPhase: number
): void {
